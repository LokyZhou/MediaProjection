...
public class FolderPickerDialogBuilder extends AlertDialog.Builder {
    ...
    private void update() {
        try {
            ...
            Log.w(Util.TAG,"Directory root is incorrect, fixing to external storage.");
        ...
        if (dirs == null) {
            Log.w(Util.TAG,"Unable to receive dirs list, no Access rights?");
            Log.d(Util.TAG,"Unable to fix, continue with empty list");
            dirs = new String[]{};
        }
        mAdapter.add("..");
